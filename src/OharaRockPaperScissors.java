...
public class OharaRockPaperScissors {
	...
	/*
	 * Difficulties: This game is programmed with four unique levels of
	 * 'difficulty' The levels are normal, harder, brutal, and impossible. You
	 * may notice there is no 'easy' mode. Life is hard. Get over it.
	 * 
	 ...
	 * always result in a loss. Your only hope is to keep your choices as varied
	 * as possible
	 * 
	 * BRUTAL: The computer can see your choice before it makes its own. In any
	 * given turn, the computer has a 65% chance to cheat outright, and
	 * automatically select the weapon that will defeat you. In the other 35% of
	 * cases, the computer will select a weapon based on normal rules. Here,
	 * victory is still very possible, but the odds are most definitely against
	 * you.
	 * 
	...
	public static int getNumGames() {
		...
		do {
			try {
				do {
					System.out
							.print("Best out of how many games (must be odd)? ");
					num = console.nextInt();
					if (num % 2 == 0 || num < 0)
						System.out
								.println("Invalid number of games. Type a positive odd number!");
				} while (num % 2 == 0 || num < 0);
				success = true; // we're done here
			} catch (InputMismatchException e) {
				System.out.println("Invalid input; must be an integer");
				console.nextLine(); // ignore the non-integer, and remove it
									// from the heap
			}
		} while ( !success);
		return num;
	...
	public static int playGame(int gameNum) {
		...
		System.out.println("I chose the weapon: "
				+ computerWeapon.toUpperCase());
	...
	public static String getUserWeapon() {
		...
		do {
			...
			isValid = weapon.startsWith("r") || weapon.startsWith("p")
					|| weapon.startsWith("s");
			if ( !isValid)
				System.out
						.println("Weapon must be rock, paper, or scissors. \""
								+ weapon + "\" is not valid.");
		} while ( !isValid);
	...
	public static String getComputerWeapon(String userInput) {
		...
		if (gamemode == Difficulty.NORMAL) {
			...
			if (userInputs[0] >= userInputs[1]
				...
				return "p";
			if (userInputs[1] >= userInputs[0]
					&& userInputs[1] >= userInputs[2])
				return "s";
			if (userInputs[2] >= userInputs[0]
					&& userInputs[2] >= userInputs[1])
				return "r";
	...
	public static boolean isValidWeapon(String weapon) {
		...
		return weapon.startsWith("r") || weapon.startsWith("p")
				|| weapon.startsWith("s");
	}
