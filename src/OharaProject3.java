...
public class OharaProject3 {
	...
	public static double setHomework(Scanner console) {
		...
		int assignments = getInputInteger(
				"How many homework assignments were there? ", 4, console);
		int[] points = new int[2];
		int cumPoints = 0; // cumulative points
		int cumPossible = 0; // cumulative possible points
		for (int i = 1; i <= assignments; i++) {
			points = getInputInteger(
					"Homework " + i + " score and max score: ", 4, 2, console);
			cumPoints += points[0];
			cumPossible += points[1];
		}
		hwScore = ((double) weight / 100) * (double) cumPoints / cumPossible;
		System.out.println("    Weighted homework score: "
				+ scoreToPercent(hwScore));
	...
	public static double setMidterm(Scanner console) {
		...
		System.out.println("    Weighted exam score: "
				+ scoreToPercent(exScore));
	...
	public static void setFinal(double hwScore, double exScore, Scanner console) {
		...
		System.out
				.print("    Have you taken the final exam yet? (1 for yes, 2 for no) ");
		...
		System.out.println("    Weighted exam score: "
				+ scoreToPercent(finScore)); // (double)10*((int)this.finScore)/10);
		// that mess of an expression is basically this.finScore as a percent to
		// one decimal place
		System.out.println();
		System.out.println("    Your course grade is "
				+ scoreToPercent(score(hwScore, exScore, finScore)));
	}
	
	// useful if the user wants to do calculations without a final grade
	public static void calcFinalReq(double hwScore, double exScore,
			Scanner console) {
		double finScore;
		int weight = getInputInteger("What is its weight (0-100)? ", 4, console);
		int goal = getInputInteger(
				"What percentage would you like to earn in the course? ", 4,
				console);
		double reqWeightedFinal = ((double) goal / 100)
				- hwMdScore(hwScore, exScore);
		double reqFinalScore = reqWeightedFinal / ((double) weight / 100);
		finScore = reqFinalScore;
		reqFinalScore = scoreToPercent(reqFinalScore);
		System.out.println();
		System.out.println("    You need a score of " + reqFinalScore
				+ " on the final exam.");
		if (reqFinalScore > 100) {
			finScore = (double) weight / 100;
			System.out
					.println("    Sorry, it is impossible to achieve this percentage.");
	...
	public static int[] getInputInteger(String prompt, int padding,
			int inputsToGet, Scanner console) {
